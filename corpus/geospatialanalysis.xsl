<xsl:stylesheet 
    ...
    <xsl:variable name="coordinates" as="map(xs:string, xs:string)">
        <xsl:map>
            ...
            <xsl:map-entry key="'Shrine of the Blessed Mother''" select="'40.431545, -79.957543'</xsl:map>
            <xsl:map-entry key="'Heinz Memorial Chapel" select="'40°26′43″N 79°57′06″W'</xsl:map-entry>
    </xsl:variable>
